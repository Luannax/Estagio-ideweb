...
function iniciarReconhecimentoVoz() {
    ...
    if (comando.includes('salvar')) {
        salvarCodigo(); // Chama a função que pede o nome por voz
        return;
    }
...
//Função para salvar um código por reconhecimento de voz
...
// Função para salvar o código diretamente pelo click do mouse
function salvarCodigoDireto() {
    var codigo = ace.edit("code").getValue();
    var nomeArquivo = "codigo_" + Date.now(); // Nome padrão com timestamp
    var blob = new Blob([codigo], { type: "text/plain;charset=utf-8" });
    var url = URL.createObjectURL(blob);
    var link = document.createElement('a');
    link.download = nomeArquivo + '.' + (linguagemAtual === 'Python' ? 'py' : 'js');
    link.href = url;
    link.click();
    URL.revokeObjectURL(url);
}
